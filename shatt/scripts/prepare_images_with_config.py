...
from shatt.dataset.images import get_image_paths
from shatt.dataset.images.preprocessing.preprocess import get_preprocessing_tfrecord_file, \
    preprocess_images_and_write_tfrecord
from shatt.dataset.images.feature_maps.distributed import create_many_feature_map_files_from_config
from shatt.scripts import OPTION_DRY_RUN
from shatt import SPLIT_VALIDATE, SPLIT_TRAIN
from shatt.dataset.images.attention_maps.distributed import create_many_attention_map_files_from_config


def main():
    parser = ArgumentParser("Prepare the MSCOCO dataset for training adn experiments")
    parser.add_argument("command", help="""One of [preprocess, featuremaps, boxes, all]. 
                        ...
                        bxoes: Create the bounding box files based on the dataset instances file. Required for experiment.
    ...
    print("\nStarting image preparation: {}".format(run_opts.command))
    directory_path = config.getDatasetImagesDirectoryPath()
    
    if run_opts.command in ["all", "preprocess"]:
        print("\nPerform preprocessing for splits: " + str(split_names))
        for split_name in split_names:
            ...
            if tfrecord_file:
                ...
                image_paths = get_image_paths(to_split_dir(directory_path, split_name))
    ...
    if run_opts.command in ["all", "featuremaps"]:
        print("\nCreated feature map files for splits " + str(split_names))
        for split_name in split_names:
            tfrecord_file = get_preprocessing_tfrecord_file(directory_path, split_name)
            if tfrecord_file:
    ...
    if run_opts.command in ["all", "boxes"]:
        print("\nCreate bounding boxes for splits: " + str(split_names))
        for split_name in split_names:
            create_many_attention_map_files_from_config(config, split_name)
