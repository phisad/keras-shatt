...
def store_tsv_to(tsv_content, directory_or_file, lookup_filename=None):
    file_path = determine_file_path(directory_or_file, lookup_filename, to_read=False)
    print("Persisting TSV to " + file_path)    
    tsv_content_header = []
    if len(tsv_content) > 0:
        tsv_content_header = tsv_content[0].keys()
    with open(file_path, "w") as tsv_file:
        dict_writer = csv.DictWriter(tsv_file, tsv_content_header, delimiter='\t', lineterminator="\n")
        dict_writer.writeheader()
        dict_writer.writerows(tsv_content)  
    return file_path
