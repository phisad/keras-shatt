...
from shatt.dataset.images import load_numpy_from, to_image_path_by_id, \
    store_numpy_to, get_infix_from_config
from shatt.dataset.images.preprocessing.tfrecords import make_dataset
from shatt.model.visual_embeddings import image_features_model
from shatt.dataset.images.preprocessing import get_tfrecord_filename
from shatt.scripts import OPTION_DRY_RUN
from shatt import SPLIT_VALIDATE, SPLIT_TRAIN, SPLIT_TEST, SPLIT_TEST_DEV
...
def create_many_feature_map_files_from_config(config, split_name):
    ...
    return create_many_feature_map_files(images_top_directory, output_layer, image_prefix, image_features_size, target_shape, batch_size, split_name, expected_num_images, is_dryrun)
...
def create_many_feature_map_files(directory_path,
    ...
    with tensorflow.Session() as sess:
        model = image_features_model(output_layer, image_features_size, input_shape, image_top_layer=False, image_top_layer_dropout_rate=0.0)
        try:
            while True:
