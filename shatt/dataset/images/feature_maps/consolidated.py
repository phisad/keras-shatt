...
from shatt.model.visual_embeddings import image_features_model
import numpy as np
from shatt.scripts import OPTION_DRY_RUN

DEFAULT_FEATURES_FILE_NAME = "shatt_feature_maps.npy"
...
def __store_feature_maps_by_image_id_to_single(image_ids, feature_maps, target_directory_path_or_file, split_name):
    ...
    if split_name:
        lookup_file_name = "shatt_feature_maps_{}.npy".format(split_name)
...
def create_single_feature_maps_file_from_config(config, split_names):
    ...
    for split_name in split_names:
        ...
        feature_map_file = create_single_feature_maps_file(output_layer, image_paths, images_top_directory, image_features_size, target_shape, split_name, config.run_opts)
        feature_map_files.append(feature_map_file)
    return feature_map_files 


def create_single_feature_maps_file(output_layer,image_paths, target_directory_path_or_file, image_features_size, target_shape, split_name=None, run_opts={}):
    """
        Create a single big numpy based feature map file containing all image id and feature map pairs.
    """
    print(run_opts)
    model = image_features_model(output_layer, image_features_size, target_shape, image_top_layer=False, image_top_layer_dropout_rate=0.0)
    ...
    image_ids = extract_to_image_ids_ordered(image_paths)
